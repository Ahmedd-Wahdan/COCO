...
#         col_matrix_size_bytes = col_matrix.nbytes

#         #conv_output is of shape (B, output_channels, H_out, W_out)

#         return conv_output , col_matrix_size_bytes
    

# Define input data and kernels
np.random.seed(0)
input_data = np.random.rand(2, 3, 32, 32)
grad = np.random.rand(2, 5, 30, 30)
# kernels = np.random.rand(5, 3, 3, 3)
convlayer = Conv2d(3, 5, 3, padding=0, stride=1)
out = convlayer.forward(input_data)

# IF YOU WANT TO TRY THE NEXT LINE THEN MAKE THE BACKWARD RETURN THEM
din,dkernel,dbias = convlayer.backward(grad)
print("din shape",din.shape)
print("dkernel shape",dkernel.shape)
print("dbias shape",dbias.shape)
print("convlayer out shape",out.shape)
# print("convlayer din shape",din.shape)
# print("convlayer dkernel shape",dkernel.shape)
...
# start2 = time.time()
# conv_output_fastest, col_matrix_size = fastest_convolver.convolve(input_data, kernels, stride=1, padding=0)
# end2 = time.time()
# input_data_torch = torch.tensor(input_data).float()
# kernels_torch = torch.tensor(kernels).float()
# start3 = time.time()	
# conv_output_torch = F.conv2d(input_data_torch, kernels_torch, stride=1, padding=0)
# end3 = time.time()
# print("fast convolver shape",conv_output_fast.shape)
# print("torch shape",conv_output_torch.shape)
# print("fastest convolver shape",conv_output_fastest.shape)
# print("col matrix size in megabytes",col_matrix_size/1024/1024)


# print("Time taken by FastConvolver: ", end1 - start1)
# print("Time taken by FastestConvolver: ", end2 - start2)
# print("Time taken by torch: ", end3 - start3)